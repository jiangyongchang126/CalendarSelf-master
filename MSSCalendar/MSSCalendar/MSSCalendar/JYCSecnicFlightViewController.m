//
//  JYCSecnicFlightViewController.m
//  MSSCalendar
//
...
@interface JYCSecnicFlightViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation JYCSecnicFlightViewController
...
- (void)initDataSource
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSArray *tempDataArray = [manager getJYCCalendarDataSoruceWithLimitMonth:_limitMonth type:_type calendarType:_calendarType  priceArray:_priceArray];
...
- (void)addWeakView
{
    ...
    for(i = 0; i < 7;i++)
    {
        UILabel *weekLabel = [[UILabel alloc]initWithFrame:CGRectMake(i * width, 20, width, MSS_WeekViewHeight-20)];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell)
    {
        ...
        if(calendarItem.day > 0)
        {
            ...
            if (self.calendarType == MSSCalendarSecnicType) {
                if (calendarItem.price.length > 0) {
                    if ([calendarItem.price floatValue] >= 0) {
            ...
            }else if(self.calendarType == MSSCalendarPlaneType){
                if (calendarItem.price.length > 0) {
                    cell.subLabel.text = [NSString stringWithFormat:@"%@",calendarItem.price];
                }else{
                    cell.subLabel.text = @"";
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.calendarType == MSSCalendarPlaneType) {
        ...
        if([_delegate respondsToSelector:@selector(calendarClickWithStartDate:)])
        {
            [_delegate calendarClickWithStartDate:_startDate];
        }
    ...
    }else if(self.calendarType == MSSCalendarSecnicType){
